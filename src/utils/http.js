...
instance.interceptors.request.use((config) => {
  ...
  if (NODE_ENV === 'production') {
    //proxy_url全局环境变量
    if(proxy_url==='test'){
      if(proxy==='activitysqx'){
        //打包测试环境activitysqx 去掉activitysqx标识，坑!!!
        config.url = `${hostEnv[proxy]}${config.url.replace('/activitysqx','')}`
      }else if(proxy==="vipcenter"){
        config.url = `${hostEnv[proxy]}${config.url.replace('/vipcenter','')}`
      }
    }else{
...
const $http = {
  ...
  async get(url, params, tag = true, isTip = true,returnRes = false) {
    try {
      let res = await instance.get(url, {
        params: params || ''
      })
      return responseRes(res.data, isTip,returnRes)
    } catch (err) {
      console.log(err)
      return errorTip(err)
    }
...
// 处理响应结果
function responseRes(res, isTip,returnRes) {
  // 清除加载
