<template>
  <div class="page_wrap" ref="page_wrap">
    <div class="main">
      <div class="stc sct1">
        <img class="sct_img" src="./img/pc/head.png" alt="" />
        <!-- <div class="swiper_l"></div> -->
        <div class="swiper-container stc1_content swiper-container1">
          <div class="swiper-wrapper">
            <div
              class="swiper-slide u-fx-ver"
              v-for="(item, index) in stc1SwiperUrls"
              :key="index"
            >
              <img :src="item.img" />
            </div>
          </div>
          <div class="swiper-pagination swiper-pagination1"></div>
        </div>
        <!-- <div class="swiper_r"></div> -->
      </div>

      <div class="stc sct2 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img2.png" alt="" />
      </div>

      <div class="stc sct3 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img3.png" alt="" />
      </div>

      <div class="stc sct4 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img4.png" alt="" />
      </div>

      <div class="stc sct5 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img5.png" alt="" />
      </div>

      <div class="stc sct6 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img6.png" alt="" />
      </div>

      <div class="stc sct6_1 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/sct6_1.png" alt="" />
      </div>

      <div class="stc sct7 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img7.png" alt="" />
      </div>

      <div class="stc sct8 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img8.png" alt="" />
      </div>

      <div class="stc sct9 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img9.png" alt="" />
      </div>

      <div class="stc sct10 u-fx-ac-jc">
        <img class="sct_img" src="./img/pc/img10.png" alt="" />
      </div>

      <div class="footer none u-fx-ac-jc" v-if="isFooter">
        <img class="footer_img sct_img" src="./img/pc/footer.png" alt="" />
        <div class="btn" @click="go68"></div>
      </div>
    </div>
    <!-- 客服 -->
    <div class="msg">
      <a href="javascript:;" @click="openMsg" class="btn_kf"></a>
    </div>
    <div
      class="mask mask_kf u-fx-ac-jc"
      v-if="isDialogue"
      @click="(e) => closeMask(e, 'isDialogue')"
    >
      <div class="pop_box">
        <div class="btn_close" @click="isDialogue = false"></div>
      </div>
    </div>

    <!-- 68权限弹窗 -->
    <div
      class="mask study u-fx-ac-jc"
      v-if="showOrdermMak"
      @click="(e) => closeMask(e, 'showOrdermMak')"
    >
      <div class="pop popod">
        <img class="pop_img" src="./img/pc_mask.png" alt="" />
        <div class="pop_close pointer" @click="showOrdermMak = false"></div>
      </div>
    </div>
  </div>
</template>

<script>
import Swiper from "swiper";
import { getOrderInfo } from "@u/orderSearch";
import { pcCheckLogin } from "@u/login";
import "@ass/js/remPc.js";
export default {
  name: "pc",
  components: {},
  data() {
    return {
      stc1SwiperUrls: [
        {
          img: require("./img/swiper1.png"),
        },
        {
          img: require("./img/swiper2.png"),
        },
        {
          img: require("./img/swiper3.png"),
        },
        {
          img: require("./img/swiper4.png"),
        },
      ],
      showOrdermMak: false,
      isBuy: false,
      isFooter: false,
      buyUrl: "",
      productId: "",
      isDialogue: false,
    };
  },
  mounted() {
    this.initPage();
  },
  async created() {
    await this.loadJsSdk();
    let { uid } = await pcCheckLogin(this.$tools.isTerminal("isInGNNPC"));
    this.uid = uid;
    this.isBuy68();
  },
  methods: {
    initPage() {
      window.addEventListener("scroll", this.showFooter, true);
      new Swiper(".swiper-container1", {
        loop: true,
        pagination: ".swiper-pagination1",
        effect: "coverflow",
        grabCursor: true,
        centeredSlides: true,
        nextButton: ".swiper-button-next",
        prevButton: ".swiper-button-prev",
        slidesPerView: "auto",
        coverflow: {
          rotate: 0, // 旋转的角度
          stretch: 68, // 拉伸   图片间左右的间距和密集度
          depth: 150, // 深度   切换图片间上下的间距和密集度
          modifier: 3, // 修正值 该值越大前面的效果越明显
          slideShadows: false, // 页面阴影效果
          slidesPerView: 3,
        },
        autoplay: 4000,
      });
      if (this.$tools.isTerminal("isInGNNApp")) {
        this.buyUrl =
          "https://uppay.upchina.com/unifiedpay/80a4b61d126df6fdef2fb800efa0adbd";
        this.productId = "UPCFJC_20211122154352";
      } else {
        this.buyUrl =
          "https://uppay.upchina.com/unifiedpay/89f12bb0bfc0f3b26caa59588047d84b";
        this.productId = "UPCFJC_20211122154510";
      }
    },
    async isBuy68() {
      if (!this.uid) return;
      let res = await getOrderInfo(this.uid, this.productId);
      this.showOrdermMak = res || false;
      // this.showOrdermMak = true;
    },
    openMsg() {
      this.isDialogue = true;
    },
    //关闭弹窗
    closeMask(e, val) {
      if (e.target._prevClass && e.target._prevClass.includes("mask")) {
        this[val] = false;
      }
    },
    //加载脚本
    loadJsSdk() {
      return new Promise((resolve) => {
        var target = document.getElementsByTagName("script")[0];
        var js = document.createElement("script");
        if (this.$tools.isTerminal("isInGNNPC")) {
          js.src =
            "https://cdn.upchina.com/activitypage/plugin/webNativeCommunicate.pc.min.js";
        } else if (this.$tools.isTerminal("isInGPTPC")) {
          js.src =
            "https://cdn.upchinaproduct.com/assert/jssdk/jssdk-pc-1.0.1.min.js";
        }
        target.parentNode.insertBefore(js, target);
        js.onload = () => {
          resolve(true);
        };
      });
    },
    //滚动第二屏展示footer
    showFooter(e) {
      let scrollY = this.$refs.page_wrap.scrollTop;
      this.isFooter = scrollY > 800;
    },
    go68() {
      let isParmas = this.$tools.getChUrl(this.buyUrl).includes("?");
      let cbUrl = window.location.href;
      window.open(
        `${this.$tools.getChUrl(this.buyUrl)}${
          isParmas ? "&" : "?"
        }callback=${encodeURIComponent(cbUrl)}`
      );
    },
  },
  destroyed() {
    window.removeEventListener("scroll", this.showFooter);
  },
};
</script>
<style lang="less" scoped>
.green {
  border: 1px solid green;
}
.page_wrap {
  overflow-y: auto !important;
  height: 9.6rem !important;
  margin: 0 auto;
  position: relative;
  font-size: 0.28rem;
  .main {
    width: 100%;
    background: #980e05 url("./img/pc/bg.png") no-repeat;
    // background: url('./img/pc/bg.png') no-repeat;
    // background-size: 100% auto;
  }
  .stc {
    width: 100%;
    position: relative;
    padding: 0.2rem;
    // background: #980e05;
    .sct_img {
      width: 11.49rem;
    }
    .swiper-container {
      overflow: visible;
      .swiper-slide {
        padding: 0.05rem;
      }
      .wiper-pagination {
        overflow: visible;
        width: 100%;
        .swiper-pagination-bullet-active {
          z-index: 100;
          background: #fff !important;
          opacity: 1;
        }
      }
      .swiper-pagination {
        bottom: -0.4rem;
      }
      img {
        width: 100%;
        height: 100%;
      }
    }
  }
  .sct1 {
    position: relative;
    padding: 0px;
    width: 100%;
    height: 9rem;
    .sct_img {
      width: 100%;
    }
    .stc1_content {
      // background: #6e130c;
      width: 11rem;
      height: 4.58rem;
      position: absolute;
      bottom: -2.9rem;
      left: 50%;
      transform: translate(-50%, 0%);
      overflow: hidden;
      .swiper-wrapper {
        // overflow: visible;
        // padding-left: 3.35rem;
      }
      .swiper-slide {
        background-position: center;
        background-size: 130% 120%;
        width: 60%;
        height: 4.58rem;
        img {
          display: block;
          width: 100%;
          height: 4.5rem;
          // width: 70%;
          // border-radius: 0.16rem;
        }
      }
    }
  }
  .sct2 {
    .sct_img {
      width: 11.24rem;
    }
    margin-top: 3.11rem;
  }
  .sct3 {
    padding: 0.1rem;
  }
  .sct4 {
    padding: 0.1rem;
  }
  .sct5 {
  }
  .sct6 {
  }
  .sct7 {
  }
  .sct8 {
  }
  .sct9 {
  }
  .sct10 {
    margin-bottom: 1.21rem;
  }
  .footer {
    position: fixed;
    z-index: 10;
    left: 0;
    right: 0;
    bottom: 0;
    height: 1.83rem;
    .footer_img {
      width: 100%;
    }
    .btn {
      position: absolute;
      right: 50%;
      transform: translate(5.5rem, 0%);
      bottom: 0;
      width: 2.1rem;
      height: 0.49rem;
    }
  }
}
// 客服
.msg {
  bottom: 2rem;
  display: block;
  position: fixed;
  left: 50%;
  transform: translate(5.5rem, 0);
  z-index: 900;
  &.hear {
    bottom: 4rem;
    .btn_kf {
      background: url("https://cdn.upchinaproduct.com/acm/202111/20211116/image/hear.png")
        no-repeat;
      background-size: 100%;
    }
  }
  .btn_kf {
    width: 1.66rem;
    height: 1.34rem;
    background: url(https://upcdnfiles.oss-cn-hangzhou.aliyuncs.com/acm/202109/aghxzbtyywx/img/btn_kf.png)
      no-repeat;
    background-size: 100%;
    display: block;
    margin: 20px auto 0;
  }
}
.mask {
  position: fixed;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background: rgba(0, 0, 0, 0.6);
  z-index: 1000;
  &.mask_kf {
    .pop_box {
      margin-top: 0.5rem;
      width: 6.75rem;
      height: 9.62rem;
      background: url("https://cdn.upchinaproduct.com/acm/202111/20211116/image/onlineUser_mak.png")
        no-repeat;
      background-size: 100%;
      position: relative;
      .btn_close {
        width: 0.6rem;
        height: 0.6rem;
        background-size: 100%;
        position: absolute;
        top: 0.1rem;
        right: 0.1rem;
        cursor: pointer;
      }
    }
  }
  &.study {
    .popod {
      width: 8rem;
      height: 8rem;
      margin-top: 1rem;
      position: relative;
      border-radius: 7px;
      overflow: hidden;
      // background: rgba(0, 0, 0, 0.6);
      .pop_img {
        width: 100%;
        height: 100%;
      }
      .btn_study {
        cursor: pointer;
        position: absolute;
        left: 50%;
        bottom: 0.64rem;
        transform: translate(-50%, 0%);
        width: 4.09rem;
        height: 0.5rem;
        background: linear-gradient(315deg, #de211e 0%, #ff610c 100%);
        box-shadow: 0rem 0.02rem 0.06rem 0rem rgba(93, 93, 93, 0.4);
        border-radius: 0.45rem;
        font-size: 0.21rem;
        color: #ffffff;
        font-weight: 500;
      }
      .pop_close {
        position: absolute;
        width: 0.4rem;
        height: 0.4rem;
        top: 0.12rem;
        right: 0.05rem;
      }
    }
  }
}
</style>
